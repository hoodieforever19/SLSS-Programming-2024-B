...
class Snowflake(pg.sprite.Sprite):
    def __init__(self, size: int):
        ...
        # Spawn it at a random x-coordinate at the top of the screen
        self.rect.centerx = random.randrange(0, WIDTH + 1)
        self.rect.centery = random.randrange(-HEIGHT, 0)  # Start off-screen above

        # Speed of the snowflake
        self.speed = random.range(1,2,3)
    ...
    def update(self):
        ...
        if self.rect.top > HEIGHT:
            self.rect.centerx = random.randrange(0, WIDTH + 1)
            self.rect.centery = random.randrange(-HEIGHT, 0)  # Reset off-screen above
